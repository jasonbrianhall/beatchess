...
ChessAIMoveResult chess_ai_compute_move(ChessGameState *game, ChessAIConfig config) {
    ...
    /* Phase 1: Find the best move using minimax */
    ...
    DEBUG_PRINT("Threshold range: %d to %d\n", 
               we_are_white ? best_score - config.threshold_centipawns : best_score + config.threshold_centipawns,
               we_are_white ? best_score : best_score);
